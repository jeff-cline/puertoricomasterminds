...
export default async function ExcursionsCmsPage() {
  ...
  const { data } = await (supabase as any).from("excursions").select("*").order("sort_order");
  return (
    <div className="space-y-6">
      <header className="flex items-end justify-between">
        ...
        <Link href="/admin/cms/excursions/new">
          <Button className="bg-prm-coral hover:bg-prm-coral/90 text-white">New excursion</Button>
        </Link>
      </header>
